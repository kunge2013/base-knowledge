...
4. **配套文章**: 后续基于这些规范文档，撰写了配套的实战指南文章 [openspec-practical-guide.md](https://github.com/ForceInjection/OpenSpec-practise/blob/main/docs/openspec-practical-guide.md) 。
...
1. **领域层 (Domain)**: 定义纯粹的类型定义 (JSDoc) [types.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/src/domain/types.js) ，无任何外部依赖。
2. **基础设施层 (Infra)**: 实现了内存版的 [memoryRepo.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/src/repo/memoryRepo.js) ，符合接口隔离原则。
3. **服务层 (Service)**: 实现了 [order.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/src/services/order.js) 等核心逻辑，负责编排领域对象。
4. **接口层 (Interface)**: 实现了 [server.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/src/http/server.js) ，处理 HTTP 协议解析与错误映射。
...
1. **集成测试 (E2E)**: [integration.spec.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/__tests__/integration.spec.js) 模拟了用户从"上架商品"到"下单成功（PENDING\_PAYMENT）"的全链路操作，验证业务逻辑闭环。
2. **性能基线**: [performance.spec.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/__tests__/performance.spec.js) 测量了接口的 p50/p99 延迟，确保满足 Spec 中的性能指标（输出 p99 < 100ms）。
3. **单元测试**: [unit.spec.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/__tests__/unit.spec.js) 覆盖了库存扣减等边界情况。
...
1. **持久化扩展**: 引入 [fileStore.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/src/persist/fileStore.js) 替换内存 Repo，接口契约保持不变。
2. **鉴权与安全**: 在 [server.prod.js](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini/src/http/server.prod.js) 中增加了 `Bearer Token` 鉴权中间件（简化实现；完整 HMAC 签名逻辑可在此处扩展）。
...
> 请读取 [domain-model/spec.md](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/openspec/changes/v1-mvp/specs/domain-model/spec.md) 。 请基于此 Spec，使用 Python 的 Pydantic 库定义 Domain Models。 要求：
> 
> 1. 文件路径： [src/domain/models.py](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini-python/src/domain/models.py)
...
*注：通过 `alias` 实现了 Python 命名规范（snake\_case）与 API 契约（camelCase）的解耦，确保内外一致性。*
...
> 请基于 [src/domain/models.py](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini-python/src/domain/models.py) 和 [design.md](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/openspec/changes/v1-mvp/design.md) ，实现 Service 层。 包含三个服务：
> 
> 1. `CatalogService`: 处理库存扣减（原子性模拟）。
> 2. `CartService`: 处理购物车增删。
> 3. `OrderService`: 编排下单流程（检查购物车 -> 扣库存 -> 生成订单 -> 清空购物车）。 需实现一个简单的 `MemoryRepo` 用于数据存储。
...
> 请基于 `src/services/` 和 `specs/catalog-management/spec.md` 、 `specs/order-management/spec.md` 等能力规范，使用 FastAPI 实现 Web 接口。 要求：
> 
> 1. 映射所有 Spec 定义的路由。
...
> 请编写 [tests/test\_smoke.py](https://github.com/ForceInjection/OpenSpec-practise/blob/main/examples/ecommerce-mini-python/tests/test_smoke.py) ，使用 `pytest` 模拟用户行为。 测试场景：
> 
> 1. 正常下单流程：加购 -> 下单 -> 验库存。
...
> **快速导航** ：想了解 CLI 命令细节？→ [OpenSpec 使用手册](https://github.com/ForceInjection/OpenSpec-practise/blob/main/docs/openspec-user-manual.md) 　|　想了解规范如何落地为代码？→ [实战指南](https://github.com/ForceInjection/OpenSpec-practise/blob/main/docs/openspec-practical-guide.md) 　|　想复盘 AI 协作过程？→ 本文档

- OpenSpec CLI 参考: [OpenSpec 使用手册](https://github.com/ForceInjection/OpenSpec-practise/blob/main/docs/openspec-user-manual.md) （init、validate、archive 等命令详解）
- 原始 Spec 指南: [openspec-practical-guide.md](https://github.com/ForceInjection/OpenSpec-practise/blob/main/docs/openspec-practical-guide.md)
...
- 演示文稿: [OpenSpec 使用手册](https://github.com/ForceInjection/OpenSpec-practise/blob/main/docs/openspec-user-manual.pptx) （适合培训与分享）