...
规范驱动开发（Specification-Driven Development, _SDD_）_是_一种以“规范为先”的软件开发方法论
...
- Spec-first 工作流：先规范、后实现，避免提示词漂移（prompt
	#### Prompt（提示词）
	输入给 AI 模型的指令或文本。
	drift）。
...
1. CLI
	...
	层（入口为 `bin/openspec.js` ，使用 Commander.js 做命令路由）。
...
![图 2: OpenSpec 顶层架构图](https://assets.jimmysong.io/images/book/ai-handbook/sdd/openspec/4e550d5cc45bb5aa31e92fc97d9e6d26.svg)
...
| 典型适用场景 | 需要强审计、合规、离线运行的小到中型团队或受限环境。 | 希望把规范直接驱动 CI/CD  #### CI/CD（持续集成/持续部署）  一种通过在应用开发阶段引入自动化来频繁向客户交付应用的方法。  、代码/测试生成的大型工程化团队（尤其 GitHub 优先）。 |
...
OpenSpec 从 SpecKit 等以人为主的规范管理工具演进而来，推动 SDD

#### SDD（规范驱动开发）

一种开发方法，其中规范驱动主要的开发流程。

（规范驱动开发）向“人机协作”演化。规范不再是单向文档，而是可执行、可验证，由 AI 与人类共同维护的工程系统。OpenSpec 提供了实现这一愿景的模式与工程工具链示例。

创建于 2025/11/03 更新于 2025/11/03 3373 字 阅读约 7 分钟

[参考文献](https://jimmysong.io/zh/book/ai-handbook/sdd/openspec/)
