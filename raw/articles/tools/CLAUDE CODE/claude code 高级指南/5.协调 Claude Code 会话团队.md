...
![比较 subagent 和 agent team 架构的图表。Subagents 由主代理生成、执行工作并报告结果。Agent teams 通过共享任务列表进行协调，队友彼此直接通信。](https://mintcdn.com/claude-code/nsvRFSDNfpSU5nT7/images/subagents-vs-agent-teams-light.png?w=2500&fit=max&auto=format&n=nsvRFSDNfpSU5nT7&q=85&s=e402533fc9e8b5e8d26a835cc4aa1742)

比较 subagent 和 agent team 架构的图表。Subagents 由主代理生成、执行工作并报告结果。Agent teams 通过共享任务列表进行协调，队友彼此直接通信。

![比较 subagent 和 agent team 架构的图表。Subagents 由主代理生成、执行工作并报告结果。Agent teams 通过共享任务列表进行协调，队友彼此直接通信。](https://mintcdn.com/claude-code/nsvRFSDNfpSU5nT7/images/subagents-vs-agent-teams-dark.png?w=2500&fit=max&auto=format&n=nsvRFSDNfpSU5nT7&q=85&s=3be00c56c6a0dcccbe15640020be0128)
...
~~~ text
我正在设计一款 CLI 工具，旨在帮助开发者在整个代码库中追踪 TODO 注释。请组建一个代理团队，从不同视角对该设计进行探讨：一名成员负责用户体验（UX），一名负责技术架构，另一名担任“魔鬼代言人”（提出质疑与反方观点）的角色。
~~~
...
## 总结：Subagent vs Agent Team 完整对比

### 核心定位一句话

| | 一句话总结 |
|---|---|
| **Subagent** | 专业员工向老板汇报结果——单向通信，主代理管控一切 |
| **Agent Team** | 团队成员互相讨论、自我协调、共同完成任务——多向通信，共享任务列表 |

### 各自解决的核心问题

#### Subagent：上下文隔离 + 专业化分工

| 痛点 | Subagent 方案 |
|------|---------------|
| 大量输出灌入主对话（测试日志、搜索结果） | 子任务在独立 context window 中运行，只返回摘要 |
| 需要约束副工具的权限 | 通过 `tools` 字段控制允许/禁止的工具 |
| 需要复用特定角色配置 | 自定义 subagent 定义文件，跨项目/跨会话复用 |
| 需要跨会话持久化知识 | `memory` 字段支持 project/user/local 三种记忆范围 |
| 需要控制成本 | 可将简单任务路由到 Haiku 模型 |

#### Agent Team：多代理协作 + 自我协调

| 痛点 | Agent Team 方案 |
|------|-----------------|
| 需要多个代理之间共享发现、相互讨论 | 队友直接互相发送消息（mailbox 系统） |
| 需要自我协调任务分配 | 共享任务列表，队友可以自我认领任务 |
| 需要对抗性探索（竞争假设） | 多个队友并行测试不同理论并相互质疑 |
| 需要跨层协调（前端、后端、测试各有人负责） | 每个队友负责一个独立层 |
| 需要计划审批 | 队友在只读计划模式下工作，负责人审批后才实施 |

### 架构对比

```
Subagent 架构：                    Agent Team 架构：
...
| ----------- | -------------------------------- | --------------------------------------------- |
| **上下文**     | 独立 context window；结果汇总回主会话       | 独立 context window；完全独立运行                      |
| **通信方式**    | 仅向主代理报告，彼此不交谈                    | 队友直接互相发消息，负责人自动接收                             |
| **协调方式**    | 主代理管理所有工作                        | 共享任务列表，自我认领 + 负责人分配                           |
| **任务依赖**    | 不支持，由主代理顺序编排                     | 支持，任务可设置依赖关系，自动解除阻塞                           |
| **权限继承**    | 继承父会话权限，可额外限制                    | 从负责人权限开始，生成后不可单独设置                            |
| **模型选择**    | 每 subagent 可独立指定模型               | 可指定队友模型，默认不继承负责人选择                            |
| **持久化记忆**   | 支持 `memory` 字段                   | 不直接支持                                         |
| **工具限制**    | 精细控制 `tools` / `disallowedTools` | 可引用 subagent 定义，但 hooks/mcpServers/skills 不生效 |
| **Hook 支持** | 完整支持 PreToolUse/PostToolUse/Stop | 支持 TeammateIdle/TaskCreated/TaskCompleted     |
| **Fork 模式** | 支持（继承完整对话历史）                     | 不支持                                           |
| **嵌套**      | 不能嵌套                             | 不能嵌套（队友不能生成自己的团队）                             |
| **令牌成本**    | 较低：结果汇总回主 context                | 较高：每个队友是独立 Claude 实例                          |
| **显示模式**    | 内嵌在主会话中                          | In-process 或 Split-pane（tmux/iTerm2）          |
| **会话恢复**    | 支持，通过 agent ID 恢复                | In-process 队友**不支持**会话恢复                      |
| **实验状态**    | 稳定功能                             | **实验性功能**，需手动启用                               |
| **计划审批**    | 不支持                              | 支持                                            |
...
|------|----------|----------------|
| 主对话 | 基准 1x | 低 |
| Subagent (Haiku) | ~0.3x | 极低 |
| Subagent (Sonnet) | ~1x/个 | 中 |
| Agent Team (3队友) | ~3-4x | 高 |
| Agent Team (5队友) | ~5-6x | 很高 |
...
├── 简单问答？           → 用 /btw
├── 可以并行但无需协调？  → 多个 Subagent 并行
└── 需要频繁对话迭代？   → 留在主对话
```

**典型场景：** 大量输出隔离（测试日志）、只读代码审查、Bug 修复、只读数据库查询、成本控制（Haiku 路由）、链式工作流（顺序编排）、项目规范共享。

#### 什么时候用 Agent Team

```
需要多代理协作吗？
├── 需要代理间讨论/辩论？  → 用 Agent Team
├── 需要自我协调任务？     → 用 Agent Team
├── 需要计划审批？         → 用 Agent Team
├── 需要对抗性探索？       → 用 Agent Team
├── 跨层协调？             → 用 Agent Team
└── 只是需要结果汇总？     → 用 Subagent
```

**典型场景：** 并行代码审查（安全+性能+测试）、对抗性调试（多假设辩论）、新模块开发（独立模块并行）、跨层协调（前端+后端+测试）、研究和探索（UX+架构+反面论证）。

#### 什么时候都不用

- 快速、简单的单步操作 → 直接在主对话完成
- 同一文件的连续编辑 → 主对话更高效
- 有大量顺序依赖的任务 → 主对话或 Subagent 链式调用
- 日常小任务 → 单个会话更具成本效益

### 最佳实践速查

| 原则 | Subagent | Agent Team |
|------|----------|------------|
| 团队规模 | 按需，无硬性限制 | 3-5 个队友最佳 |
| 任务粒度 | 自包含、可返回摘要 | 自包含单元（函数、测试文件、审查） |
| 每人任务数 | - | 5-6 个/队友 |
| 避免冲突 | 不适用 | 分解工作使每人拥有不同文件集 |
| 监控频率 | 检查返回摘要 | 定期检查进度，无人值守太久有风险 |
| 新手起点 | 只读审查、文件搜索 | 从研究和审查开始，不要一上来就写代码 |
...
[Agent view](https://code.claude.com/docs/zh-CN/agent-view) [使用动态工作流大规模编排子代理](https://code.claude.com/docs/zh-CN/workflows)